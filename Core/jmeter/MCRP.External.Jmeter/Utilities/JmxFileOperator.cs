...
namespace MCRP.External.Jmeter.Utilities
{
    ...
    public class JmxFileOperator:IJmxFileOperator
    {
        ...
        /// </summary>
        /// <returns></returns>
        protected async  Task CheckDefaultlBasicFolder(string id="") {
            await  Task.Run(()=> {           
        ...
        {           
            await CheckDefaultlBasicFolder(id);
        ...
        protected async Task<bool> DealUploadFile(string id,string fileName){
             ...
             return await Task<bool>.Run( async ()=>{                
                   ...
                   switch(_extension){
                    //    case ".jmx":
                    //         //重命令文件                            
                    //         targetFileName=Path.Combine(JMX_ROOT_PATH,id,"start-up.jmx");
                    //         if(File.Exists(Path.Combine(JMX_ROOT_PATH,id,fileName)))
                  ...
                  return  await  ProcessDataFilePath(targetFileName);
             });
        }
        ...
        protected async Task<bool> ProcessDataFilePath(string jmxFilePath)
        {
                    ...
                    for (int i = 0; i < matches.Count; i++)
                    {
                        ...
                        {                              
                            ...
                            {        
                                ...
                                fileText = fileText.Replace(oldPath, $"<stringProp name=\"filename\">{truePath}</stringProp>");
                            }
                        }
                    }
                }
